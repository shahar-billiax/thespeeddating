...
import { getUpcomingRegisteredEvents } from "@/lib/dashboard/actions";
      ...
      <Tabs defaultValue="upcoming">
        <TabsList className="h-10 p-1">
          <TabsTrigger value="upcoming" className="gap-1.5 px-4">
            {t("dashboard.events_tab_upcoming")}
            {upcomingEvents.length > 0 && (
              <span className="ms-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-blue-100 px-1.5 text-[11px] font-semibold text-blue-700">
                {upcomingEvents.length}
              </span>
            )}
          </TabsTrigger>
          <TabsTrigger value="past" className="gap-1.5 px-4">
            {t("dashboard.events_tab_past")}
            {pastEvents.length > 0 && (
              <span className="ms-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-muted px-1.5 text-[11px] font-semibold text-muted-foreground">
                {pastEvents.length}
              </span>
            )}
          </TabsTrigger>
          ...
          {pastEvents.length === 0 ? (
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center py-12 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Attend an event to see it here
                </p>
              </CardContent>
            </Card>
          ) : (
    ...
    <Card className="group overflow-hidden transition-all hover:shadow-md hover:border-primary/20">
      <CardContent className="p-0">
        <div className="flex items-stretch">
          ...
          {/* Content */}
          <div className="flex flex-1 flex-col justify-center gap-2 border-s border-border/50 p-4">
...
async function PastEventCard({ event }: { event: any }) {
  const t = await getTranslations();
  const locale = await getLocale();
  let progress = null;
    ...
    <Card className="group overflow-hidden transition-all hover:shadow-md hover:border-primary/20">
      <CardContent className="p-0">
        <div className="flex items-stretch">
          ...
          {/* Content */}
          <div className="flex flex-1 flex-col gap-2 border-s border-border/50 p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-base leading-snug">
                  {event.type
