...
export default async function DashboardResultsPage({
  ...
  const friendMatches = data.matches!.filter((m) => m.type === "mutual_friend");
  ...
  let compatScores: Record<
    string,
    { score: number; dealbreakers_passed: boolean }
  > = {};
  try {
    const profileIds = data
      .matches!.map((m: any) => m.userId)
      .filter(Boolean);
          ...
          <div className="space-y-3">
            {dateMatches.map((match) => (
              <MatchCard
                ...
                match={match}
                type="date"
                compatScore={compatScores[(match as any).userId]}
                labels={{ date: t("matches.date"), friend: t("matches.friend"), compatible: t("dashboard.compatible_percent", { score: "{score}" }), passesDealbreakers: t("dashboard.passes_dealbreakers"), noContacts: t("matches.no_shared_contacts") }}
              />
            ))}
          </div>
        </div>
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-0.5">
                  ...
                  {match.age && match.city && <span>&middot;</span>}
                  {match.city && (
